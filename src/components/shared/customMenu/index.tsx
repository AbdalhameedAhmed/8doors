...
export default function CustomMenu({ children, icon,containerStyle }: customMenuType) {
  ...
  return (
    <div className={classNames("p-1 rounded-full relative w-[45px] h-[45px] flex items-center justify-center hover:bg-layout-secondary", { "bg-layout-secondary": subMenu })} onClick={() => {
      menuHandler()
    }}
      ref={menuRef}
    >
      ...
      ></span>
      <div className={classNames("absolute top-[42px] shadow-lg bg-white rounded-xl right-0 w-[220px] transition-all duration-500 opacity-0 scale-0 origin-[90%_0%] max-h-[300px] overflow-y-scroll snap-y snap-center scrollbar-hide p-3", {
        "scale-100 opacity-100": subMenu
      },containerStyle)}
        style={{
