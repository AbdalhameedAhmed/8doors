  ...
  return (
    <li
      className={classNames(
        "flex flex-col w-full cursor-pointer relative text-4 capitalize group px-4",
        {
    ...
    >
      <a
        ...
        className={classNames(
          "flex justify-between items-center py-[14px] px-[10px] flex-row group-hover:bg-layout-secondary rounded-lg ease-in-out w-full ",
          className,
          {
      ...
      >
        <div className={classNames(textContainerStyle, "flex gap-4 items-center justify-center ", { "!flex-col !gap-2": smallView })}>
          <div className="relative">
            {!removeIcon && (icon)}
            {item.submenu?.length !== 0 && (<span className={classNames("absolute top-[5px] -translate-y-1/2 -right-[15px] w-[5px] h-[5px] block", { "hidden": !smallView })}><RightAngle classNames="w-[5px] h-[5px] fill-black" /></span>)
            }
          </div>
