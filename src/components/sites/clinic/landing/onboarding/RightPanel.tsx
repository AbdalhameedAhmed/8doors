...
export default function RightPanel({ boardingList, activeItem, changeActiveItem, nestedActiveForm, changeNestedActiveForm, direction }: rightPanelTypes) {
              ...
              <ul>
                {
                  ...
                  boardingList?.map((itemInfo, index) => (
                    <li key={index} onClick={() => { handelItemClick(index) }}>
                      ...
                      <div className={`${styles.onboardingProgress} ${direction === "ltr" ? "mr-[20px]" : "ml-[20px]"} ${index <= activeItem && `${styles.active}`}`}>
                        <span>{index + 1}</span>
                      </div>
